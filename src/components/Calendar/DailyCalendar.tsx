...
export function DailyCalendar() {
  ...
  const eventsForCurrentDay = getEventsForDay(currentDate);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-2 h-full border rounded-md overflow-hidden">
        {/* Time labels column */}
        ...
        <div>
          <div className="h-12 border-b flex flex-col items-center justify-center">
            <div className="font-medium">{format(currentDate, "EEEE", { locale: fr })}</div>
          </div>
          
          {/* Time slots with events */}
          {timeSlots.map((time) => {
            const eventsAtTime = eventsForCurrentDay.filter(
